...
**This section describes the OUTGOING design and is being replaced.** The app is mid-redesign on `redesign-2` — see `docs/design/stampd-redesign-2026/IMPLEMENTATION-PLAN.md` for the phase plan and `Stampd - Foundations.dc.html` for the new token values. The old reference screens (`frontend design/`) were deleted deliberately: the redesign was briefed from scratch without the stamp metaphor, so the old prototype is a source of confusion, not truth.

The new system is **"editorial ledger"**: `--bg #F7F8F7` / `--surface #FFFFFF` / `--ink #14201C` / `--primary #0FA968`, Space Grotesk (display) + DM Serif Display (numerals only) + Inter (body) + IBM Plex Mono (keys/slugs). Its one rule: **`--primary` green means value and action, `--brand` means tenant identity, and the two never swap jobs** — a customer must not lose track of "how much can I spend" because an outlet picked an unfortunate brand colour. `lib/color.ts` derives every tenant token through a measured contrast check and detects a tenant brand that is itself green (identity steps aside to the ink; the value green never moves). `scripts/verify-tenant-color.ts` guards both invariants — run it after touching `lib/color.ts`. All animation config lives in `lib/motion.ts` and is only reachable through `useMotion()`, which resolves `useReducedMotion` for you.
